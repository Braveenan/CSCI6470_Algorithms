...
def generate_config1_file():
    ...
    Includes randomly chosen alpha and beta scores (-3 to -1).
    """
    ensure_config_dir()
    labels = ['A', 'C', 'G', 'T']
    ...
    with open("config/config1.txt", "w") as f:
        ...
        alpha_score = random.randint(-3, -1)
        beta_score = random.randint(-3, -1)
        f.write(f"alpha\t{alpha_score}\n")
        f.write(f"beta\t{beta_score}\n")
...
def generate_config2_file():
    ...
    Includes alpha and beta scores (-3 to -1).
    """
    ensure_config_dir()
    labels = ['A', 'C', 'G', 'T']
    combinations = itertools.product(labels, repeat=2)
